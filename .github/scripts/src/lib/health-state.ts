...
export const HEALTH_CHECK_JOB = 'queue-health-check'
export const UNHEALTHY_ALERT_OUTPUT = 'should_alert_unhealthy'
export const RECOVERY_ALERT_OUTPUT = 'should_alert_recovery'
 ...
 */
export function inferPreviousHealthFromJobs(jobs: WorkflowJob[]): PreviousHealth {
  const job = jobs.find((candidate) => candidate.name === HEALTH_CHECK_JOB)
 ...
 */
export function decideAlert(currentHealth: CurrentHealth, previousHealth: PreviousHealth): AlertKind {
  if (currentHealth === 'unhealthy' && (previousHealth === 'healthy' || previousHealth === 'unknown')) {
    return 'unhealthy'
  }
