...
describe('inferPreviousHealthFromJobs', () => {
  ...
  it('returns unknown when there are no recognizable steps', () => {
    ...
    expect(inferPreviousHealthFromJobs([{ name: 'queue-health-check', steps: [] }])).toBe('unknown')
    expect(inferPreviousHealthFromJobs([{ name: 'queue-health-check', steps: null }])).toBe('unknown')
  })
