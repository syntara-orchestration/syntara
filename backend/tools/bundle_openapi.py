...
# Domains containing internal-only endpoints (/_internal/*).
# Excluded when generating the public-facing spec.
_INTERNAL_DOMAINS = {
    "internal_metrics",
}
...
DEFAULT_PUBLIC_OUTPUT = SCHEMAS_DIR / "openapi-public.yaml"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--output",
        default=None,
        help=f"Output file path (default: {DEFAULT_OUTPUT}, or {DEFAULT_PUBLIC_OUTPUT} with --public)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
# =============================================================================
# DO NOT EDIT — this file is generated by `make api-spec-bundle-public`.
#
# Source: tools/bundle_openapi.py --public
# Sub-specs: src/syntara/schemas/*/openapi.yaml  (excluding internal domains)
#
# This is the PUBLIC-FACING spec matching what the runtime API exposes.
# Internal endpoints (/_internal/*) are excluded.
#
# To update: edit the relevant domain sub-spec, then run:
#   make api-spec-bundle-public
# =============================================================================
"""
...
def main() -> None:
    ...
    if args.list_specs:
        ...
        print(f"Discovered {label}sub-specs:")
        for p in sub_specs:
    ...
    label = "public " if args.public else ""
    print(f"Bundling {len(sub_specs)} {label}sub-spec(s)...")
    for p in sub_specs:
        print(f"  {p.relative_to(PROJECT_ROOT)}")
