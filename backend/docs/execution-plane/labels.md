...
| Volume mounts, CPU/memory requests as *live* resources | Work payload / Worker Manager; Extension metadata | The reconciler must not inspect volume or network *state* of a target in order to match. Constructing the mount is execution, not placement. See [example 02](examples/02-data-sharing-with-workspace.md). |
