...
| `approval_node_id` | str | Activity ID from workflow definition. Inside a loop this is `{node_id}_iter_{n}` (or `{node_id}_iter_{outer}_iter_{inner}` when nested) so each iteration is unique. |
...
1. **Validate uniqueness**: Ensures no approval already exists for this `(execution_id, approval_node_id)` pair. When the approval node is inside a loop body, `approval_node_id` is `{canvas_node_id}_iter_{index}` (and `{canvas_node_id}_iter_{outer}_iter_{inner}` for nested loops) so each iteration can create its own request.
