...
flowchart TD
    ...
    P1 --> P2["Phase 2: Start SyntaraWorkflow via Temporal<br/>(no DB session held during RPC)"]
...
2. **Temporal phase** — starts `SyntaraWorkflow` (no DB session held during the RPC call)
