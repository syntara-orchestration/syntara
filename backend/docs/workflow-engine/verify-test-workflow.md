...
| Template expressions | `invalid_reference` | Validates `${...}` expressions reference valid scopes (`input`, `inputs`, `trigger`, `workflow_context`), existing node IDs, or `loop` (only inside loop bodies). Detects `${loop.*}` used outside loop bodies. |
