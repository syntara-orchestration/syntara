...
| Template expressions | `invalid_reference` | Validates `${...}` expressions reference valid scopes (`trigger`, `workflow_context`), existing node IDs, or `loop` (only inside loop bodies). Rejects leftover V1 scopes (`input`, `inputs`, `variables`) and `${loop.*}` used outside loop bodies. This is a breaking change: leftover scopes that previously published now 409 on the next publish. |
