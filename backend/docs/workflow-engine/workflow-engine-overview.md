...
2. **Namespace registration.** `_process_pending_tasks()` calls `resolver.set_namespace(node_id, output)` (`namespace_resolver.py`). `NamespaceResolver` keeps one dict-of-dicts keyed by node ID, plus the reserved `trigger`, `input`/`inputs` (aliases of the trigger payload), `workflow_context`, and `loop` namespaces.
