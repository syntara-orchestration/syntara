...
Loop *control* activities use Temporal IDs `{loop_id}_iter_{n}`, or `{loop_id}_iter_{outer}_iter_{inner}` when that loop is itself inside another loop, so Temporal never reuses an activity ID when an inner loop resets on the next outer iteration. Approval nodes **inside** a loop body reuse that suffix for both the Temporal `activity_id` and the Approvals API `approval_node_id` (`{node_id}_iter_{n}`, or `{node_id}_iter_{outer}_iter_{inner}` when loops are nested). That keeps the `(execution_id, approval_node_id)` unique constraint satisfied across iterations — including when an inner loop resets its index — and lets the decision signal complete the in-flight activity. Activity sync strips every `_iter_{n}` suffix back to the canvas node ID; only LOOP-typed nodes with that suffix are treated as loop control.
