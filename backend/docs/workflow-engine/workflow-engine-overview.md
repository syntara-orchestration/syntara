...
Loop *control* activities already use Temporal IDs `{loop_id}_iter_{n}`. Approval nodes **inside** a loop body reuse that suffix for both the Temporal `activity_id` and the Approvals API `approval_node_id` (`{node_id}_iter_{n}`). That keeps the `(execution_id, approval_node_id)` unique constraint satisfied across iterations and lets the decision signal complete the in-flight activity. Activity sync strips `_iter_{n}` back to the canvas node ID; only LOOP-typed nodes with that suffix are treated as loop control.
