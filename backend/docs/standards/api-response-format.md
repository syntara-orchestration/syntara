...
`in` and `isnull` are auto-generated by `FilterableModel` (`in` for all fields, `isnull` for Optional fields) but are not required by the compliance test suite. See [step 5 under Adding a New List Endpoint](#adding-a-new-list-endpoint) for the full operator set breakdown.
...
5. **Add `FilterableModel` dependency** — this generates deepObject-style filter params in the exported OpenAPI spec from model metadata, so the drift checker validates them:
   ...
   - **String fields**: `eq`, `in`, `contains`, `starts_with`, `gt`, `gte`, `lt`, `lte`
   - **Boolean/enum/UUID fields**: `eq`, `in`
   - **Datetime fields**: `eq`, `in`, `gt`, `gte`, `lt`, `lte`
   - **Numeric fields** (int/float): `eq`, `in`, `gt`, `gte`, `lt`, `lte`
   - **Optional fields** (any type): adds `isnull` to the type's base operator set

   The `in` operator always uses a `string` schema (comma-separated values) and `isnull` always uses a `boolean` schema, regardless of the field's base type. The compliance test suite does not require `in` or `isnull` — endpoints may declare them but are not penalized for omitting them.
