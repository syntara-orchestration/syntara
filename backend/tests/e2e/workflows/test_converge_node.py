...
@pytest.mark.e2e
@pytest.mark.parametrize("strategy_label", ["any_n1"])
def test_converge_any_does_not_cancel_nodes_outside_parallel_section(
    syntara_api: SyntaraApiRegistry,
    strategy_label: str,
) -> None:
    """Converge ANY (n=1) must not cancel side_action attached to the slow branch.
    ...
    When fast_branch completes first, converge fires immediately (n_required=1).
    slow_branch is still in-flight (detached) — it is NOT skipped.
    When slow_branch eventually finishes, side_action must be scheduled and run.

    Before the fix, if slow_branch had not yet been scheduled (multi-step chain
    variant), side_action was incorrectly added to skipped_nodes by the unbounded
    _mark_downstream_as_skipped propagation.  The bounded replacement
    (_mark_downstream_as_skipped_bounded) restricts propagation to the parallel
    section, leaving side_action untouched.
    """
    result = create_and_run_workflow(
        syntara_api,
        f"e2e-converge-any-no-cancel-outside-{strategy_label}",
        {
            "name": "converge-any-no-cancel-outside-parallel-section",
            "schema_version": "2.0.0",
            "triggers": [{"id": "trigger", "type": "manual_trigger", "parameters": {}}],
            "nodes": [
                {
                ...
                {
                    "id": "side_action",
                    "name": "Side Action (must not be cancelled by converge)",
                    "type": "script",
            ...
            "edges": [
                ...
                # side_action branches off slow_branch — outside the converge parallel section
    ...
    # Converge fired on the fast branch — downstream must complete normally
    assert result.status == ExecutionStatus.COMPLETED, f"Workflow failed: {result.status}"
    assert activities["fast_branch"].status == "completed"
    assert activities["converge_node"].status == "completed"
    assert activities["final_action"].status == "completed"

    # slow_branch was detached (in-flight when converge fired); it eventually
    # finishes and side_action is scheduled from it — side_action must run.
    assert activities["slow_branch"].status == "completed", "slow_branch should complete as a detached execution"
    assert activities["side_action"].status == "completed", (
        "side_action must not be cancelled by the converge mechanism; "
        "it is outside the parallel section and should run after slow_branch"
    )
