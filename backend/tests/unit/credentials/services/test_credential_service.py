...
class TestFetchLatestExecutions:
    ...
    @pytest.mark.asyncio
    async def test_skips_null_workflow_ids_in_results(self, service: CredentialService) -> None:
        """Null workflow_id rows are excluded from the result map."""
        from datetime import UTC, datetime

        now = datetime.now(UTC)
        mock_result = MagicMock()
        mock_result.all.return_value = [(None, now, "completed")]
        service.session.exec = AsyncMock(return_value=mock_result)  # type: ignore[method-assign]
