...
async def test_upload_sets_content_hash() -> None:
    ...
    fm = FileManager()
    mock_retriever = AsyncMock()
    mock_retriever.save_file = AsyncMock(return_value="syntara-uuid-hash_test.txt")
    fm._retriever = mock_retriever
...
async def test_multiple_files_saved_successfully() -> None:
    ...
    async def unique_save(content: bytes, path: str) -> str:
        nonlocal call_count
        call_count += 1
        return f"syntara-{call_count}-file.pdf"
...
async def test_storage_failure_cleans_up_saved_files() -> None:
    ...
    mock_retriever.delete_file.assert_called_once_with("syntara-1-file0.pdf")
...
async def test_file_upload_events_logged() -> None:
    ...
    with patch("syntara.files.file_manager.logger") as mock_logger:
        fm = FileManager()
        mock_retriever = AsyncMock()
        mock_retriever.save_file = AsyncMock(return_value="syntara-uuid-logged.pdf")
        fm._retriever = mock_retriever
...
async def test_async_io_used_for_file_operations() -> None:
    ...
    fm = FileManager()
    mock_retriever = AsyncMock()
    mock_retriever.save_file = AsyncMock(return_value="syntara-uuid-async_test.pdf")
    fm._retriever = mock_retriever
...
async def test_load_file_with_integrity_check_success() -> None:
    ...
    mock_metadata = Mock()
    mock_metadata.file_path = "syntara-uuid-integrity-check.txt"
...
async def test_load_file_with_integrity_check_no_hash() -> None:
    ...
    mock_metadata = Mock()
    mock_metadata.file_path = "syntara-uuid-legacy.txt"
...
async def test_load_file_with_integrity_check_hash_mismatch() -> None:
    ...
    mock_metadata = Mock()
    mock_metadata.file_path = "syntara-uuid-tampered.txt"
...
async def test_audit_event_storage_backend_is_s3() -> None:
    ...
    fm = FileManager()
    mock_retriever = AsyncMock()
    mock_retriever.save_file = AsyncMock(return_value="syntara-uuid-audit.txt")
    fm._retriever = mock_retriever
