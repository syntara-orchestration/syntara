...
PERMISSION_CHECK_QUERY_RESULT = {
    "check_node": {"status": "completed", "allowed": False, "checked_node_id": "denied_node", "kind": "script"}
}


class TestSyncPermissionCheckNodes:
    """_sync_permission_check_nodes writes COMPLETED with the routing output."""
    ...
    def _activity(self, name: str) -> Mock:
        return TestSyncDeniedNodes._activity(self, name)  # type: ignore[arg-type]

    def _mock_session(self, activities: list[Mock]) -> Mock:
        return TestSyncDeniedNodes._mock_session(self, activities)  # type: ignore[arg-type]

    @pytest.mark.asyncio
    async def test_permission_check_marked_completed_with_output(self) -> None:
        activity = self._activity("check_node")
        self._mock_session([activity])
        metadata = create_test_metadata(execution_id=self.execution_id, activity_index_map={"check_node": 1})
        handle = AsyncMock()
        handle.query = AsyncMock(return_value=PERMISSION_CHECK_QUERY_RESULT)

        with patch.object(self.service, "_publish_activity_patches", new_callable=AsyncMock):
            await self.service._sync_permission_check_nodes(metadata, handle)

        handle.query.assert_awaited_once_with("get_permission_check_results")
        assert activity.status == ActivityStatus.COMPLETED
        assert activity.output_data == PERMISSION_CHECK_QUERY_RESULT["check_node"]
        assert activity.started_at is not None
        assert activity.completed_at is not None
        assert activity.error_details is None

    @pytest.mark.asyncio
    async def test_no_results_is_a_noop(self) -> None:
        metadata = create_test_metadata(execution_id=self.execution_id, activity_index_map={})
        handle = AsyncMock()
        handle.query = AsyncMock(return_value={})
