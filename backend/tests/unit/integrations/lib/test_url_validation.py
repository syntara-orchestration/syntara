"""Tests for integrations/lib/url_validation.validate_url_no_ssrf()."""
...
class TestValidateUrlNoSsrf:
    """Tests for validate_url_no_ssrf() — integration base_url SSRF checks."""
    ...
    def test_public_ip_accepted(self) -> None:
        ...
        with patch(_PATCH_GETADDRINFO, return_value=_mock_getaddrinfo(_PUBLIC_IP)):
            validate_url_no_ssrf("https://aap.example.com")

    def test_localhost_accepted(self) -> None:
        """Always accept localhost (loopback) so local integrations work without config."""
        validate_url_no_ssrf("http://localhost:8765", allow_http=True)

    def test_loopback_ipv4_accepted(self) -> None:
        """Always accept 127.0.0.0/8 loopback."""
        validate_url_no_ssrf("http://127.0.0.1:8080", allow_http=True)

    def test_loopback_ipv6_accepted(self) -> None:
        """Always accept ::1 loopback."""
        validate_url_no_ssrf("http://[::1]:8080", allow_http=True)

    def test_cloud_metadata_ipv4_rejected(self) -> None:
        """Reject AWS/GCP cloud metadata endpoint."""
        with pytest.raises(ValueError, match="SSRF blocked"):
            validate_url_no_ssrf("http://169.254.169.254", allow_http=True)

    def test_cloud_metadata_ipv6_rejected(self) -> None:
        """Reject AWS IPv6 cloud metadata endpoint."""
        with pytest.raises(ValueError, match="SSRF blocked"):
            validate_url_no_ssrf("http://[fd00:ec2::254]", allow_http=True)
    ...
    def test_kubernetes_internal_dns_rejected(self) -> None:
        ...
        with (
            ...
            validate_url_no_ssrf("http://kubernetes.default.svc", allow_http=True)

    def test_private_ip_rejected(self) -> None:
        """Reject RFC1918 private IP without allowlist."""
        with pytest.raises(ValueError, match="SSRF blocked"):
            validate_url_no_ssrf("http://192.168.1.100", allow_http=True)
    ...
    def test_https_required_by_default(self) -> None:
        ...
        with (
            ...
            validate_url_no_ssrf("http://aap.example.com")
    ...
    def test_allowlisted_private_host_accepted(self) -> None:
        ...
        with (
            ...
            validate_url_no_ssrf("http://aap.internal.corp", allow_http=True)
