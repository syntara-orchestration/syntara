...
| --- | --- | --- |
| `quay.io/ahetheri/script-bash-executor:amd64` | Bash | Bash only |
| `quay.io/ahetheri/script-python-executor:amd64` | Python | Python standard library only |
...
docker tag syntara-script-bash-executor:dev quay.io/ahetheri/script-bash-executor:amd64
docker tag syntara-script-python-executor:dev quay.io/ahetheri/script-python-executor:amd64
docker push quay.io/ahetheri/script-bash-executor:amd64
docker push quay.io/ahetheri/script-python-executor:amd64
```
...
## Run as cold-start OpenShift workers

The Execution Plane creates a short-lived pod with the selected image, keeps
its main process alive, and invokes the one-shot command with Kubernetes
`pods/exec`:
...
See [cold-start development](../../docs/execution-plane/cold-start-development.md)
for the pod lifecycle and image settings used in this POC.
