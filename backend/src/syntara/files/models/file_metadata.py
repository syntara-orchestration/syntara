...
class FileMetadata(BaseResource, table=True):
    ...
    Attributes:
        ...
        file_path: Internal storage path (pattern: syntara-{file_id}-{filename})
    ...
    file_path: str = Field(
        max_length=500,
        description="Internal storage path: syntara-{file_id}-{filename}",
    )
    converted_content_path: str | None = Field(
        default=None,
        max_length=500,
        description="Path to converted markdown: syntara-{file_id}-content.md",
    )
