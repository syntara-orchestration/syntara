...
def validate_labels_dict(labels: dict[str, str] | None) -> dict[str, str] | None:
    ...
    if len(labels) > FieldLimits.MAX_LABELS_COUNT:
        msg = (
            f"labels contains {len(labels)} entries, maximum is {FieldLimits.MAX_LABELS_COUNT}"
        )
        raise SafeValueError(msg)
    ...
    for key, value in labels.items():
        ...
        if len(key) > FieldLimits.MAX_LABEL_KEY_LENGTH:
            msg = (
                f"labels key exceeds maximum length of {FieldLimits.MAX_LABEL_KEY_LENGTH} characters"
            )
            raise SafeValueError(msg)
