...
Strategy: disassociate executions (SET NULL) rather than cascade-delete,
pending Decision 1. Versions cascade with their parent workflow.
...
def upgrade() -> None:
    ...
    # CUSTOM: Disassociate executions referencing soft-deleted workflows before purge.
    op.execute(
        sa.text(
            "UPDATE executions SET workflow_id = NULL, workflow_version_id = NULL "
            "WHERE workflow_id IN (SELECT id FROM workflows WHERE deleted_at IS NOT NULL)"
        )
    )

    # CUSTOM: Disassociate executions referencing soft-deleted workflow versions.
    op.execute(
        sa.text(
            "UPDATE executions SET workflow_version_id = NULL "
            "WHERE workflow_version_id IN (SELECT id FROM workflow_versions WHERE deleted_at IS NOT NULL)"
        )
    )

    # CUSTOM: Null out published_version_id for soft-deleted workflows (self-referential FK).
    op.execute(sa.text("UPDATE workflows SET published_version_id = NULL WHERE deleted_at IS NOT NULL"))
    ...
    # -- executions: change workflow_id FK from RESTRICT to SET NULL, make nullable --
    ...
    op.create_foreign_key(
        "executions_workflow_id_fkey", "executions", "workflows", ["workflow_id"], ["id"], ondelete="SET NULL"
    )

    # -- executions: change workflow_version_id FK from RESTRICT to SET NULL, make nullable --
    op.alter_column("executions", "workflow_version_id", existing_type=sa.UUID(), nullable=True)
...
def downgrade() -> None:
    ...
    op.alter_column("executions", "workflow_version_id", existing_type=sa.UUID(), nullable=False)
