...
class Execution(UserOwnedResource, table=True):
    ...
    Uses hard deletes. When a parent workflow is deleted, executions are
    preserved with workflow_id/workflow_version_id set to NULL (disassociated).
    ...
    Attributes:
        ...
        workflow_id: Foreign key to parent Workflow (nullable — NULL if workflow deleted)
        workflow_version_id: Foreign key to WorkflowVersion (nullable — NULL if version deleted)
    ...
    # Foreign keys — nullable, SET NULL when parent workflow/version is deleted
    workflow_id: UUID | None = Field(
        default=None,
        foreign_key="workflows.id",
        nullable=True,
        ondelete="SET NULL",
        description="Workflow ID being executed (NULL if workflow deleted)",
        index=True,
    )
    ...
    workflow_version_id: UUID | None = Field(
        ...
        nullable=True,
        ondelete="SET NULL",
        description="Workflow version ID executed (NULL if version deleted)",
    )
