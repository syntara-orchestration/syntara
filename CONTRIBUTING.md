...
succeed or are skipped (for example, doc-only PRs). Frontend GitHub Actions
also run when `backend/` changes, so API updates cannot land without UI
coverage.

Job names, Konflux path filters, fork behavior, and how results appear are in
[docs/ci/pr-checks.md](docs/ci/pr-checks.md). If a check fails and you cannot
re-trigger or debug it, ask a maintainer on the PR.
