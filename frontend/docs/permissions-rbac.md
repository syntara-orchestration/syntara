...
| -------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `useWorkflowPermissions`         | `routes/workflows/useWorkflowPermissions.ts`                                | create via `check_any_project` when no project selected (toolbar); update/delete/run use per-row `resourceProject` (`workflow.project_id`) |
