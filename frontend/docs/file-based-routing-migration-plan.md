...
- One intentional non-TanStack path: `/auth/test-signin-callback` is rendered
  from `App.tsx` before `RouterProvider` so the identity-provider test-signin
  popup can finish without mounting `AppShell` / the auth gate. The Phase 0
  baseline records it as `kind: "app"`. This is not a bug; fold it into a
  public/layout-less TanStack route after the migration (see Follow-ups).
...
- App-level paths handled outside the TanStack tree (today:
  `/auth/test-signin-callback` in `App.tsx`).
...
- Revisit the `App.tsx` pre-router escape hatch for
  `/auth/test-signin-callback` (see Follow-ups). Do not block Phase 3 on
  moving it; keep URL and popup behavior identical until an explicit follow-up.
...
## Follow-ups after migration (or revisit)

Track these separately from the URL-preserving migration. They are cleanups,
not Phase 0 defects.

### Move `/auth/test-signin-callback` into TanStack Router

Today this path is short-circuited in `App.tsx` before `RouterProvider` so the
OAuth test-signin popup never mounts QueryClient providers, the router, or
`AppShell` (auth gate + chrome). That behavior is intentional.

After file-based routing is stable, either:

1. **Preferred:** add a public / layout-less TanStack route for
   `/auth/test-signin-callback` that skips `AppShell` and still avoids the
   authenticated shell, then remove the `App.tsx` pathname branch and the
   baseline `kind: "app"` exception; or
2. **Revisit and keep:** if a layout-less route cannot match the popup
   constraints cleanly, document the escape hatch as a permanent exception in
   the baseline and drop the “move it” item.

Acceptance: popup still writes claims to `localStorage` and closes; no auth
gate flash; baseline no longer needs a special `app` kind unless option 2 is
chosen.
