...
import styles from './TFEIntegrationSelector.module.css'
import { TypeaheadMenuToggle } from './TypeaheadMenuToggle'

type IntegrationRead = IntegrationsAPI.components['schemas']['IntegrationRead']

export type TFEIntegrationSelectorProps = {
  value?: string
  ...
  /** When provided, filters TFE integrations to those that are global or assigned to this project. */
  projectId?: string
...
export function TFEIntegrationSelector({
  value,
  onChange,
  label = 'Integration',
  fieldId = 'tfe-integration-selector',
  isDisabled = false,
  isRequired = false,
  helpText,
  projectId,
  onStaleDetected,
}: Readonly<TFEIntegrationSelectorProps>) {
    ...
    <FormGroup label={formGroupLabel} isRequired={isRequired}>
        ...
        <SelectList>
          {integrations.length === 0 && !isPending && (
            <SelectOption isAriaDisabled value="__empty__">
              No TFE integrations configured
            </SelectOption>
          )}
      ...
      {!isPending && integrations.length === 0 && (
        <IntegrationRequiredHelper
          integrationLabel="a Terraform Enterprise integration"
          actionLabel="an integration can be selected"
        />
      )}
    </FormGroup>
  )
