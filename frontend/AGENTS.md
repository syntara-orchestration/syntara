...
17. **Use `RhUi*` icons for action buttons** -- never use PatternFly icons like `PlusCircleIcon` directly; use `RhUiAddIcon`, `RhUiDuplicateIcon`, etc. from `@patternfly/react-icons`
...
- **API Contract Generation**: Types generated from the backend OpenAPI specs in this repository, shared between UI and Mock API — update via `npm run gen`
