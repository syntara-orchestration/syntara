...
| Variable                      | Purpose                                                                                         |
| ----------------------------- | ----------------------------------------------------------------------------------------------- |
| `SYNTARA_E2E_PASSWORD`        | Admin password generated by `make admin-password` (stored in `backend/.secrets/admin-password`) |
| `VITE_API_URL`                | Backend API URL (HTTPS)                                                                         |
| `SYNTARA_E2E_BASE_URL`        | Frontend dev server URL where Playwright opens the browser                                      |
| `SYNTARA_E2E_SKIP_WEB_SERVER` | Skip Playwright's built-in web server since frontend is already running                         |

The Playwright config sets `ignoreHTTPSErrors` to `true` in real-backend mode (when `SYNTARA_E2E_SKIP_WEB_SERVER=1`) so the browser accepts the self-signed certificate. In mock-API mode it remains `false`.
