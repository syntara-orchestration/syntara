...
| New file that only re-exports from other files                 | #34 -- do not add a barrel file (coding standards §43); `barrel-files/avoid-barrel-files` is `warn`, not `error`, so also check the diff by eye |
