...
4. Use `useFormMutationErrorHandler(setError)` for API 422 field errors; Zod handles client-side only. See `frontend/docs/error-handling.md` - "Client-side validation (Zod + @hookform/resolvers)"
...
The application uses RFC 9457 Problem Details for API error responses. See `frontend/docs/error-handling.md` for complete patterns.
...
All CRUD and destructive actions must be gated by permissions. See `frontend/docs/permissions-rbac.md` for the full architecture.
...
## Documentation Links -- `useDocLink` Hook
